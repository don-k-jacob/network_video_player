...
import 'package:networkvideos/Networking/networking.dart';

class Data{
  dynamic getData() async{
    NetworkHelp networkHelp = NetworkHelp(
        url:
        'https://coronavirus-tracker-api.herokuapp.com/v2/latest');//enter your url here


    //add ur url

//    var data = await networkHelp.getData();
    var data=
    {
    "categories": [
        {
...
//    var data = await networkHelp.getData();
    print(data['categories'][0]['videos']);
  ...
  Data({this.description, this.subtitle,this.title,this.sources,this.thumb});
  }
List<Data> parseJosn(dynamic response) {
  ...
  return parsed.map<Data>((json) => new Data.fromJson(json)).toList();
}
